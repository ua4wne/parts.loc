...
@section('content')
    ...
    <ul class="breadcrumb">
        ...
        <li class="active"><a href="{{ route('methods') }}">{{ $title }}</a></li>
    </ul>
    <!-- END BREADCRUMB -->
    @if (session('error'))
        <div class="row">
                            ...
                            @foreach($rows as $k => $row)
                                <tr>
                                    ...
                                    <td style="width:100px;">
                                        {!! Form::open(['url'=>route('methodEdit',['id'=>$row->id]), 'class'=>'form-inline','method' => 'POST', 'onsubmit' => 'return confirmDelete()']) !!}
                                        {{ method_field('DELETE') }}
                                        <div class="form-group" role="group">
                                            <a href="{{route('methodEdit',['id'=>$row->id])}}">
