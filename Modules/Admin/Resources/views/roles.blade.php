...
@section('content')
    ...
    <div class="container-fluid container-fullw bg-white">
        <div class="row">
            <div class="modal fade" id="actions" tabindex="-1" role="dialog" aria-labelledby="roleAction"
                 aria-hidden="true">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-8 col-sm-8 col-xs-12" id="sel_roles">
                                    @foreach($actions as $action)
                                        <div class="checkbox">
                                            <label>
                                                <input type="checkbox" name="actions[]" value="{{ $action->code }}"
                                                       id="{{ $action->code }}"> {{ $action->name }}
                                            </label>
                                        </div>
                                    @endforeach
                                </div>
                            ...
                            @foreach($roles as $k => $role)
                                ...
                                <tr id="{{ $role->id }}">
                                    ...
                                    <td style="width:110px;">
                                        {!! Form::open(['url'=>route('roleEdit',['id'=>$role->id]), 'class'=>"form-inline",'method' => 'POST', 'onsubmit' => 'return confirmDelete()']) !!}
                                        {{ method_field('DELETE') }}
                                        <div class="form-group" role="group">
                                            <button class="btn btn-info role" type="button" data-toggle="modal"
                                                    data-target="#actions" title="Настройка разрешений"><i
                                                    class="fa fa-cogs fa-lg" aria-hidden="true"></i></button>
