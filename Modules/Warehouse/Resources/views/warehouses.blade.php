...
@section('content')
    ...
    <ul class="breadcrumb">
        ...
        <li class="active"><a href="{{ route('warehouses') }}">{{ $title }}</a></li>
    </ul>
    <!-- END BREADCRUMB -->
    @if (session('error'))
        <div class="row">
                            ...
                            @foreach($rows as $k => $row)
                                <tr>
                                    ...
                                    <td style="width:110px;">
                                        {!! Form::open(['url'=>route('warehouseEdit',['id'=>$row->id]), 'class'=>'form-inline','method' => 'POST', 'onsubmit' => 'return confirmDelete()']) !!}
                                        {{ method_field('DELETE') }}
                                        <div class="form-group" role="group">
                                            <a href="{{route('warehouseEdit',['id'=>$row->id])}}">
