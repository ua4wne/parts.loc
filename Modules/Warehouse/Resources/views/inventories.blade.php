...
@section('content')
    ...
    <ul class="breadcrumb">
        ...
        <li class="active"><a href="{{ route('inventories') }}">{{ $title }}</a></li>
    </ul>
    <!-- END BREADCRUMB -->
    @if (session('error'))
        <div class="row">
                                ...
                                @else
                                    <tr class="success">
                                        ...
                                        <td style="width:150px;">
                                            {!! Form::open(['url'=>route('inventoryEdit',['id'=>$row->id]), 'class'=>'form-inline','method' => 'POST', 'onsubmit' => 'return confirmDelete()']) !!}
                                            {{ method_field('DELETE') }}
                                            <div class="form-group" role="group">
                                                <a href="{{route('inventoryView',['id'=>$row->id])}}">
                                                    <button class="btn btn-info" type="button"
                                                            title="Открыть документ"><i class="fa fa-eye fa-lg>"
                                                                                        aria-hidden="true"></i></button>
                                                </a>
                                                @if($row->status)
                                                    <a href="{{route('inventoryEdit',['id'=>$row->id])}}">
