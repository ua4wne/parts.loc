...
@section('content')
    ...
    <ul class="breadcrumb">
        ...
        <li class="active"><a href="{{ route('orders') }}">{{ $title }}</a></li>
    </ul>
    <!-- END BREADCRUMB -->
    @if (session('error'))
        <div class="row">
                        ...
                        <table id="mytable" class="table table-bordered">
                            ...
                            @foreach($rows as $k => $row)
                                <tr {!! $row->statuse->style !!} }>
                                    ...
                                    <td>{{ $row->hoperation->title }}</td>
                                    <td>{{ $row->user->name }}</td>
                                    <td style="width:100px;">
                                        {!! Form::open(['url'=>route('orderDelete'), 'class'=>'form-inline','method' => 'POST', 'onsubmit' => 'return confirmDelete()']) !!}
                                        {!! Form::hidden('order_id',$row->id,['class' => 'form-control','required'=>'required','id'=>'order_id']) !!}
                                        <div class="form-group" role="group">
                                            <a href="{{route('orderView',['id'=>$row->id])}}">
